#!/usr/bin/env python3
"""
Final Optimized Processor - Maximum Speed with Smart File Detection:
- Automatically skips already processed files
- NVIDIA GPU: 80% of work (sustained high utilization)
- CPU cores: 20% of work (parallel processing)
- Processes only remaining files: P5-P9, P12-P31
- Real-time progress monitoring and GPU utilization tracking
"""
...
class FinalOptimizedProcessor(UltimateMultiDeviceParser):
    """Final optimized processor with smart file detection and maximum speed."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Optimized settings for GPU + CPU
        self.nvidia_batch_size = 180  # Optimized batch size
        self.cpu_batch_size = 30      # CPU batch size
        self.cpu_workers = 16         # Use 16 CPU cores
    ...
    def _setup_nvidia_gpu_optimized(self):
        """Set up NVIDIA GPU for optimized processing."""
        ...
        try:
            ...
            logging.error(f"Failed to setup NVIDIA GPU: {e}")
            return False
    
    def _setup_cpu_cores_optimized(self):
        """Set up CPU cores for optimized processing."""
        try:
            from transformers import AutoTokenizer, AutoModelForTokenClassification, pipeline
            
            logging.info(f"🔧 Setting up {self.cpu_workers} CPU cores...")
            ...
            logging.info(f"✅ Created {len(self._cpu_pipelines)} CPU processing pipelines")
            return True
    ...
    def start_progress_logging(self, total_addresses, file_name):
        ...
        def log_progress():
            while not self.stop_progress:
                time.sleep(25)  # Log every 25 seconds
                if not self.stop_progress:
                    with self.progress_lock:
                        ...
                        logging.info(f"📊 OPTIMIZED [{self.current_file}]: {self.completed_addresses:,}/{self.total_addresses:,} "
    ...
    def process_single_file_optimized(self, file_path: str) -> Dict:
        """Process a single P file with optimized GPU + CPU processing."""
        file_name = os.path.basename(file_path)
        logging.info(f"🚀 Starting OPTIMIZED processing: {file_name}")
        ...
        # Setup CPU cores
        if self._setup_cpu_cores_optimized():
        ...
        # Optimized work distribution (80% NVIDIA + 20% CPU)
        nvidia_chunk_size = int(total_addresses * 0.80)
        cpu_chunk_size = total_addresses - nvidia_chunk_size
        
        logging.info(f"🎯 OPTIMIZED work distribution for {file_name}:")
        logging.info(f"   NVIDIA RTX 4070: {nvidia_chunk_size:,} addresses (80%)")
        logging.info(f"   CPU cores ({self.cpu_workers}): {cpu_chunk_size:,} addresses (20%)")
        
        # Process with optimized distribution
        ...
        with ThreadPoolExecutor(max_workers=self.cpu_workers + 2) as executor:
            ...
            if nvidia_chunk_size > 0 and "NVIDIA_GPU" in available_devices:
                ...
                nvidia_future = executor.submit(self._process_chunk_nvidia_optimized, nvidia_chunk)
                futures.append((nvidia_future, current_idx, current_idx + nvidia_chunk_size, "NVIDIA_GPU"))
                current_idx += nvidia_chunk_size
            
            # Submit CPU work (distributed across cores)
            if cpu_chunk_size > 0 and "CPU_CORES" in available_devices:
                ...
                for i in range(self.cpu_workers):
                    ...
                    if start_idx < total_addresses:
                        ...
                        cpu_future = executor.submit(self._process_chunk_cpu_optimized, cpu_chunk, i)
        ...
        logging.info(f"🎉 {file_name} OPTIMIZED processing completed!")
    ...
    def _process_chunk_nvidia_optimized(self, addresses: List[str]) -> List[ParsedAddress]:
        """Process chunk on NVIDIA GPU with optimization."""
        logging.info(f"🟢 NVIDIA GPU starting: {len(addresses):,} addresses (80% optimized)")
        ...
        try:
            ...
            for i in range(0, len(addresses), self.nvidia_batch_size):
                ...
                # Log progress every 12 batches
                if (i // self.nvidia_batch_size) % 12 == 0:
        ...
        logging.info(f"✅ NVIDIA GPU completed: {len(addresses):,} addresses in {processing_time:.1f}s ({final_rate:.1f}/sec)")
        return results
    
    def _process_chunk_cpu_optimized(self, addresses: List[str], core_id: int) -> List[ParsedAddress]:
        """Process chunk on CPU core with optimization."""
        logging.info(f"🔵 CPU_CORE_{core_id} starting: {len(addresses):,} addresses (20% share)")
        start_time = time.time()
        ...
        processing_time = time.time() - start_time
        self._device_stats["cpu_cores"]["processed"] += len(addresses)
        self._device_stats["cpu_cores"]["time"] += processing_time
    ...
    def save_results(self, file_name: str, addresses: List[str], results: List[ParsedAddress]) -> str:
        ...
        output_file = f'{base_name}_final_optimized_{timestamp}.csv'
...
def setup_logging():
    """Set up comprehensive logging for final optimized processing."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    logging.basicConfig(
        level=logging.INFO,
        ...
        handlers=[
            ...
            logging.FileHandler(f'final_optimized_{timestamp}.log', encoding='utf-8')
        ]
    )


def main():
    """Main final optimized processing function."""
    
    setup_logging()
    
    print("🚀 FINAL OPTIMIZED PROCESSOR")
    print("Configuration: NVIDIA RTX 4070 (80%) + 16 CPU cores (20%)")
    print("Smart file detection - skips already processed files")
    print("=" * 80)
    
    # Initialize processor
    processor = FinalOptimizedProcessor(
        batch_size=180,  # Optimized batch size
        use_nvidia_gpu=True,
        use_intel_gpu=False,  # Focus on NVIDIA + CPU
        use_all_cpu_cores=True,
        cpu_core_multiplier=0.5
    )
    
    processor.cpu_workers = 16  # Use 16 CPU cores
    processor.max_workers = 18  # 16 CPU + 1 NVIDIA + 1 coordinator
    ...
    print()
    print("🔧 Initializing FINAL OPTIMIZED processing...")
    print("🎯 Target: 80% GPU + 20% CPU for maximum speed")
    print("🎯 16 CPU cores working in parallel with GPU")
    print("🎯 Optimized batch sizes and real-time monitoring")
    print()
    ...
    for i, file_path in enumerate(files_to_process, 1):
        ...
        try:
            result = processor.process_single_file_optimized(file_path)
            if result:
